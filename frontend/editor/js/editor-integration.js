async function initBackendIntegration() {
    const isAuth = await MiruFlowAPI.checkAuth();

    if (isAuth) {
        updateUserMenu();
        loadCurrentProject();
        console.log('[Integration] Пользователь авторизован:', MiruFlowAPI.getCurrentUser());
    } else {
...
function updateUserMenu() {
    const user = MiruFlowAPI.getCurrentUser();
    if (!user) return;
...
function setupProjectModal() {
    document.getElementById('create-project-btn')?.addEventListener('click', async () => {
        ...
        const result = await MiruFlowAPI.createProject(name);
...
async function loadProjectsList() {
    ...
    const projects = await MiruFlowAPI.getProjects();
...
async function selectProject(projectId, event) {
    ...
    const project = await MiruFlowAPI.loadProject(projectId);
...
async function loadCurrentProject() {
    ...
    if (projectId) {
        const project = await MiruFlowAPI.getProject(projectId);
        if (project) {
            authState.currentProject = project;
            loadProjectIntoEditor(project);
        }
...
async function handleSaveProject() {
    const isAuth = MiruFlowAPI.isAuthenticated();
    ...
    if (!isAuth) {
        ...
        if (token) {
            ...
            await MiruFlowAPI.checkAuth();
            if (MiruFlowAPI.isAuthenticated()) {
    ...
    const result = await MiruFlowAPI.saveProject(state.sections, canvasSize);
